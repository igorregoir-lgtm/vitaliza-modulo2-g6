...
| E4 | **Experiência educacional / autodescoberta** ("Simular esta alavanca"): anima a alavanca sugerida até o alvo, recalcula ao vivo e abre a explicação em texto; respeita `prefers-reduced-motion` | `live-simulator.tsx#runAutoDemo` | RM1 (PBL); §3 do spec | ☑ |
| E5 | **Explicação em linguagem natural** (determinística): em TEXTO via "Tutor Explica", com áudio OPCIONAL ("Ouvir" dentro do painel) — TTS ElevenLabs/Google com fallback p/ voz do navegador | `lib/simulator/narrate.ts#buildNarration`; `components/tutor/use-speak.ts` (extraído do tutor) | §5, §8 do spec; ADR-0013 | ☑ |
