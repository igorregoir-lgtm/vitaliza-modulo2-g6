...
- **Progresso em `localStorage`**, sem login nem servidor de estado (`use-trilha-progress` via
  `useSyncExternalStore`, SSR-safe). Chave `vitaliza:trilha:v1`.
...
- O progresso vive no navegador: simples, sem login, mas **não** sincroniza entre dispositivos
  (aceitável; é material de aprendizado, não dado de produção).
